...
This separate v6 entry preserves immutable v2/v3/v4 source identities and outputs.
...
from cifar_relative_asr_gate import select_validation, SELECTION_METRICS, describe_final_targets, ASR_TARGET, validate_policy
from cifar_expanded_candidates import run_budget

REPO = Path(__file__).resolve().parent
DEFAULT_CONFIG = REPO / "configs/cifar10_six_relative_asr_v6.json"
...
def build_manifest(spec, contract, repo):
    ...
    manifest.update(schema_version=6, source_sha256=source_hashes(repo),
...
def load_spec(path):
    ...
    if spec.get("schema_version") != 6 or set(spec["candidates"]) != set(ALL_METHODS):
        raise ValueError("schema_version=6 and exactly six methods are required")
    if spec.get("selection_metrics") != SELECTION_METRICS:
        raise ValueError("v6 requires explicit final-round Accuracy/ASR selection")
    data = spec["dataset"]
    ...
    if (type(promotion.get("require_mean_dual_best")) is not bool or
        ...
        raise ValueError("v6 requires Ours health/targets, explicit mean-best policy and no baseline health veto")
...
def validate_expanded_protocol(spec):
    ...
    policy = {"accuracy_gap": .02, "asr_gap": .01, "tail_rounds": 10}
    if spec["performance_target"] != policy:
        raise ValueError("v6 retains paired VERT limits; ASR uses the selected-method minimum")
    if spec["objective"] != {"clean_accuracy_weight": .25, "robust_accuracy_weight": .5,
                              "attack_success_weight": .2, "honest_weight_loss_weight": .05}:
        raise ValueError("v6 retains the shared CIFAR Score; target changes do not change its weights")
    for phase, seeds, ratios in (
        ...
        if (set(spec[phase]["seeds"]) != seeds or
            ...
            raise ValueError(f"v6 {phase} must retain its three seeds and ten declared scenarios")
    if any(c["variant"] != "original" for cs in spec["candidates"].values() for c in cs):
        raise ValueError("v6 searches original policies; detector development needs a separate protocol")
    ...
    with file_lock(output / ".runner.lock", nonblocking=True):
        ...
        print("OURS_FINAL_METRIC_GATE " + json.dumps(json_safe({
            key: gate.get(key) for key in ("status", "comparison_scope", "selected_candidate",
                ...
                "asr_minimum_target_passed", "asr_target", "require_mean_dual_best", "eligible_ours_candidates", "selection_metrics")
        }), ensure_ascii=False), flush=True)
...
def main(argv=None):
    ...
    if args.plan_only:
        ...
        spec = load_spec(args.config)
        output = (args.output or REPO / spec["output_dir"]).resolve()
        print(json.dumps({"schema_version": 6, "output_dir": str(output),
