...
from cifar_relative_asr_gate import SELECTION_METRICS, ASR_TARGET, select_validation
...
def reanalyze(source, output):
    ...
    spec.update(schema_version=6, selection_metrics=dict(SELECTION_METRICS), asr_target=dict(ASR_TARGET),
    ...
    selected = {**report["selected"], "sm9rrs": report["selected"].get("sm9rrs", report["best_healthy_score_candidate"])}
