#!/usr/bin/env python3
"""Independent 630-run validation study with an Ours mean-accuracy/ASR gate.

Original training, baseline algorithms and v2 entry point remain unchanged.
The worker/scheduler bodies are retained from v2; this module owns their entry
path, experiment fingerprint, validation coverage and promotion policy.
"""
...
from cifar_mean_dual_gate import select_validation

REPO = Path(__file__).resolve().parent
DEFAULT_CONFIG = REPO / "configs/cifar10_six_630_mean_v3.json"
...
def build_manifest(spec, contract, repo):
    ...
    manifest.update(schema_version=3, source_sha256=source_hashes(repo),
                    promotion_requires="ours_health_and_mean_dual_best_or_near_vert",
                    validation_run_count=630, final_run_count=180,
...
def load_spec(path):
    ...
    if spec.get("schema_version") != 3 or set(spec["candidates"]) != set(ALL_METHODS):
        raise ValueError("schema_version=3 and exactly six methods are required")
    data = spec["dataset"]
    ...
    if spec["promotion"] != {"required_healthy_methods": ["sm9rrs"],
            "performance_target_is_gate": True, "attack_effectiveness_is_gate": False,
            "missing_clean_reference": "report_unassessed_without_baseline_veto"}:
        raise ValueError("v3 requires Ours health plus mean dual-best or near-VERT promotion")
    ...
    for method, candidates in spec["candidates"].items():
        if len(candidates) != (6 if method in ("sm9rrs", "vert", "alignins") else 1):
            raise ValueError("v3 uses six candidates per tunable method and one per fixed baseline")
        for candidate in candidates:
            ids.append(candidate["candidate_id"])
            if not re.fullmatch(method + r"-v9-\d{3}", candidate["candidate_id"]):
    ...
    validate_630_protocol(spec)
    return spec


def validate_630_protocol(spec):
    expected_gate = {
        "accuracy_metric": "mean_final_accuracy_all_validation_runs",
        "asr_metric": "mean_attack_asr_attacked_validation_runs",
        "comparison": "scorable_selected_baselines_and_healthy_ours_candidates",
        "absolute_tolerance": 1e-12,
        "unavailable_baselines": "report_unassessed_without_veto",
        "acceptance": "joint_best_or_near_vert",
        "near_vert_accuracy_gap": .005,
        "near_vert_asr_gap": .01,
        "baseline_selection": "healthy_score_then_complete_score_then_fixed",
    }
    if spec.get("mean_dual_gate") != expected_gate:
        raise ValueError("v3 requires the declared near-VERT thresholds and healthy/complete-score baseline selection")
    for phase, seeds, ratios in (
        ("validation", {401, 402, 403}, {0., .1, .3, .5, .7}),
        ("final", {901, 902, 903}, {0., .2, .4, .6, .8}),
    ):
        ...
        if (set(spec[phase]["seeds"]) != seeds or
                {(s["partition"], s["malicious_ratio"]) for s in scenarios} != expected):
            raise ValueError(f"v3 {phase} must retain its three seeds and ten declared scenarios")
    ours = spec["candidates"]["sm9rrs"]
    expected_ours = {(warning, remove) for warning in (1.25, 1.75, 2.5) for remove in (3, 5)}
    if ({(c["parameters"].get("detector_distance_threshold"),
          c["parameters"].get("suspicion_remove_after")) for c in ours} != expected_ours
            or any(set(c["parameters"]) != {"detector_distance_threshold", "suspicion_remove_after"}
                   for c in ours)):
        raise ValueError("v3 restores the six original Ours threshold/removal candidates")
    if any(c["variant"] != "original" for candidates in spec["candidates"].values() for c in candidates):
        raise ValueError("v3 uses original algorithms; development variants require a new protocol")
...
def run_parent(args):
    ...
    with file_lock(output / ".runner.lock", nonblocking=True):
        ...
        print("OURS_MEAN_DUAL_GATE " + json.dumps(json_safe({
            key: gate.get(key) for key in ("status", "comparison_scope", "selected_candidate",
                                       "best_mean_accuracy", "best_mean_asr", "qualified_ours_candidates",
                                       "selected_pass_route", "near_vert_reference")
        }), ensure_ascii=False), flush=True)
        if report["status"] != "qualified_for_final":
            ...
            print("FINAL_NOT_STARTED: no healthy Ours candidate is mean-dual-best or within the declared VERT accuracy/ASR gaps; validation evidence retained.", flush=True)
            return 0
        ...
        final_report["validation_mean_dual_gate"] = {key: value for key, value in gate.items() if key != "candidate_rows"}
        final_report["validation_near_vert_gate"] = report["near_vert_gate"]
