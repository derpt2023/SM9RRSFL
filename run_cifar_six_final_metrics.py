...
This separate v5 entry preserves immutable v2/v3/v4 source identities and outputs.
...
from cifar_final_metric_gate import select_validation, SELECTION_METRICS, describe_final_targets
from cifar_expanded_candidates import run_budget

REPO = Path(__file__).resolve().parent
DEFAULT_CONFIG = REPO / "configs/cifar10_six_final_metrics_v5.json"
...
def source_hashes(repo):
    ...
    for name in ("run_cifar_six_630.py", "cifar_mean_dual_gate.py",
                 ...
                 "run_cifar_six_final_metrics.py"):
...
def build_manifest(spec, contract, repo):
    ...
    manifest.update(schema_version=5, source_sha256=source_hashes(repo),
...
def load_spec(path):
    ...
    if spec.get("schema_version") != 5 or set(spec["candidates"]) != set(ALL_METHODS):
        raise ValueError("schema_version=5 and exactly six methods are required")
    if spec.get("selection_metrics") != SELECTION_METRICS:
        raise ValueError("v5 requires explicit final-round Accuracy/ASR selection")
    data = spec["dataset"]
    ...
    if (type(promotion.get("require_mean_dual_best")) is not bool or
        ...
        raise ValueError("v5 requires Ours health/targets, explicit mean-best policy and no baseline health veto")
...
def validate_expanded_protocol(spec):
    ...
    if spec["performance_target"] != policy:
        raise ValueError("v5 retains thresholds; temporal peak/tail fields are diagnostic only")
    if spec["objective"] != {"clean_accuracy_weight": .25, "robust_accuracy_weight": .5,
                              "attack_success_weight": .2, "honest_weight_loss_weight": .05}:
        raise ValueError("v5 retains the shared CIFAR Score; target changes do not change its weights")
    for phase, seeds, ratios in (
        ...
        if (set(spec[phase]["seeds"]) != seeds or
            ...
            raise ValueError(f"v5 {phase} must retain its three seeds and ten declared scenarios")
    if any(c["variant"] != "original" for cs in spec["candidates"].values() for c in cs):
        raise ValueError("v5 searches original policies; detector development needs a separate protocol")
    ...
    with file_lock(output / ".runner.lock", nonblocking=True):
        ...
        print("OURS_FINAL_METRIC_GATE " + json.dumps(json_safe({
            key: gate.get(key) for key in ("status", "comparison_scope", "selected_candidate",
                ...
                "absolute_target_passed", "require_mean_dual_best", "eligible_ours_candidates", "selection_metrics")
        }), ensure_ascii=False), flush=True)
...
def main(argv=None):
    ...
    if args.plan_only:
        ...
        spec = load_spec(args.config)
        output = (args.output or REPO / spec["output_dir"]).resolve()
        print(json.dumps({"schema_version": 5, "output_dir": str(output),
