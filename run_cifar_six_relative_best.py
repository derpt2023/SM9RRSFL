#!/usr/bin/env python3
"""CIFAR validation with final-round performance targets and baseline fallback.

This separate v7 entry preserves immutable v2/v3/v4/v5/v6 source identities and outputs.
...
from cifar_relative_best_gate import (select_validation, SELECTION_METRICS, describe_final_targets,
                                      ASR_TARGET, ACCURACY_TARGET, validate_policy)
from cifar_expanded_candidates import run_budget

REPO = Path(__file__).resolve().parent
DEFAULT_CONFIG = REPO / "configs/cifar10_six_relative_best_five_day_v7.json"
...
def source_hashes(repo):
    ...
    for name in ("run_cifar_six_630.py", "cifar_mean_dual_gate.py",
                 ...
                 "cifar_expanded_candidates.py", "cifar_final_metric_gate.py",
                 "run_cifar_six_final_metrics.py", "cifar_relative_asr_gate.py",
                 "run_cifar_six_relative_asr.py", "cifar_relative_best_gate.py",
                 "run_cifar_six_relative_best.py"):
...
def build_manifest(spec, contract, repo):
    ...
    manifest.update(schema_version=7, source_sha256=source_hashes(repo),
                    promotion_requires="ours_health_and_final_round_scenario_targets",
...
def load_spec(path):
    ...
    if spec.get("schema_version") != 7 or set(spec["candidates"]) != set(ALL_METHODS):
        raise ValueError("schema_version=7 and exactly six methods are required")
    if spec.get("selection_metrics") != SELECTION_METRICS:
        raise ValueError("v7 requires explicit final-round Accuracy/ASR selection")
    data = spec["dataset"]
    ...
    if (type(promotion.get("require_mean_dual_best")) is not bool or
        ...
        raise ValueError("v7 requires Ours health/targets, explicit mean-best policy and no baseline health veto")
...
def validate_expanded_protocol(spec):
    ...
    policy = {"accuracy_gap": .02, "asr_gap": .02, "tail_rounds": 10}
    if spec["performance_target"] != policy:
        raise ValueError("v7 requires inclusive two-point gaps to the selected-method best for both metrics")
    if spec["objective"] != {"clean_accuracy_weight": .25, "robust_accuracy_weight": .5,
                              "attack_success_weight": .2, "honest_weight_loss_weight": .05}:
        raise ValueError("v7 retains the shared CIFAR Score; target changes do not change its weights")
    for phase, seeds, ratios in (
        ...
        if (set(spec[phase]["seeds"]) != seeds or
            ...
            raise ValueError(f"v7 {phase} must retain its three seeds and ten declared scenarios")
    if any(c["variant"] != "original" for cs in spec["candidates"].values() for c in cs):
        raise ValueError("v7 searches original policies; detector development needs a separate protocol")
...
def summarize_final(spec, selected, results, statuses, tasks, selection_details=None):
    ...
    report["process_target_diagnostics"] = report["ours_target"]
    report["process_target_diagnostics"]["role"] = "diagnostic_only"
    report["ours_target"] = describe_final_targets(spec, selected, results, tasks)
...
def run_parent(args):
    ...
    with file_lock(output / ".runner.lock", nonblocking=True):
        ...
        gate = report["final_metric_gate"]
        print("OURS_FINAL_METRIC_GATE " + json.dumps(json_safe({
            key: gate.get(key) for key in ("status", "comparison_scope", "selected_candidate",
                ...
                "asr_minimum_target_passed", "accuracy_maximum_target_passed", "asr_target", "accuracy_target", "require_mean_dual_best", "eligible_ours_candidates", "selection_metrics")
        }), ensure_ascii=False), flush=True)
        ...
        final_report["validation_final_metric_gate"] = {
...
def main(argv=None):
    ...
    if args.plan_only:
        ...
        spec = load_spec(args.config)
        output = (args.output or REPO / spec["output_dir"]).resolve()
        print(json.dumps({"schema_version": 7, "output_dir": str(output),
            ...
            "performance_target": spec["performance_target"], "selection_metrics": spec["selection_metrics"],
            "asr_target": spec["asr_target"], "accuracy_target": spec["accuracy_target"],
            "promotion": spec["promotion"],
